...
class FeaturesEmbedding(Layer):
    def __init__(self, field_dims, embed_dim, **kwargs):
        ...
        self.offsets = np.array((0, *np.cumsum(field_dims)[:-1]), dtype=np.int64)  # Store offsets as int64
    ...
    def call(self, x):
        ...
        # Ensure offsets is int64
        offsets = tf.cast(tf.constant(self.offsets), tf.int64)
        x = x + offsets
...
class AutoIntMLP(Layer): 
    def __init__(self, field_dims, embedding_size, att_layer_num=3, att_head_num=2, att_res=True, 
                 dnn_hidden_units=(64, 64), dnn_activation='relu', l2_reg_dnn=0, l2_reg_embedding=1e-5, 
                 dnn_use_bn=False, dnn_dropout=0.3, init_std=0.0001):
        ...
        self.int_layers = [MultiHeadSelfAttention(embedding_size, att_head_num, use_res=att_res) 
                           for _ in range(att_layer_num)]

        self.final_layer = Dense(1, use_bias=True, kernel_initializer=tf.random_normal_initializer(stddev=init_std))
    ...
    def call(self, inputs):
        ...
        y_pred = tf.nn.sigmoid(self.final_layer(combined_output))
